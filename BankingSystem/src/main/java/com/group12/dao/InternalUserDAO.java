...
public class InternalUserDAO {
	...
	public boolean checkIfMobileExists(String parameter) {
		...
		int count = 0;
		try {
			count = jdbcTemplate.queryForObject("select count(*) from InternalUser where mobile = " + parameter + ";",
					Integer.class);
	...
	public boolean checkIfEmailExists(String parameter) {
		...
		try {
			count = jdbcTemplate.queryForObject(
					"select count(*) from InternalUser where email = " + "'" + parameter + "'" + ";", Integer.class);
		} catch (DataAccessException ex) {
			throw new RuntimeException(ex);
		}
		if (count > 0) {
	...
	public boolean checkIfUserNameExists(String parameter) {
		...
		try {
			count = jdbcTemplate.queryForObject(
					"select count(*) from InternalUser where emp_user_id = " + "'" + parameter + "'" + ";", Integer.class);
		} catch (DataAccessException ex) {
			throw new RuntimeException(ex);
		}
		if (count > 0) {
